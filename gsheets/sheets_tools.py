...
async def get_spreadsheet_info(
    ...
    Returns:
        str: Formatted spreadsheet information including title, locale, and
            each sheet's type. Callers must treat non-GRID sheets such as
            DATA_SOURCE as metadata-only because ordinary A1 value reads are
    ...
    spreadsheet = await asyncio.to_thread(
        ...
        .get(
            ...
            fields="spreadsheetId,properties(title,locale),sheets(properties(title,sheetId,sheetType,gridProperties(rowCount,columnCount)),conditionalFormats)",
        )
        .execute
    )
    ...
    for sheet in sheets:
        ...
        sheets_info.append(
            f'  - "{sheet_name}" (ID: {sheet_id}) | Type: {sheet_type} | Size: {rows}x{cols} | Conditional formats: {len(rules)}'
        )
        if rules:
            sheets_info.append(
                _format_conditional_rules_section(
