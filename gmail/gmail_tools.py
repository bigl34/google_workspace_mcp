...
async def search_gmail_messages(
    ...
    Returns:
        A machine-readable envelope whose IDs and pagination cursor cannot be
        lost when a caller truncates the human-readable rendering. Its
        formattedResponse includes Subject, From, and Date when
    ...
    for message in messages:
        ...
        record = {
            ...
            "messageUrl": (
                _generate_gmail_web_url(message["id"]) if message.get("id") else None
            ),
            "threadUrl": (
                _generate_gmail_web_url(message["threadId"])
                if message.get("threadId")
                else None
            ),
        }
        if headers_by_id is not None:
            headers = headers_by_id.get(message.get("id"))
            record["headers"] = headers
            record["headersAvailable"] = headers is not None
    ...
    return {
        ...
        "formattedResponse": formatted_output,
    }


@server.tool(
    title="List Gmail Send-As Identities",
    annotations=ToolAnnotations(
        readOnlyHint=True,
        destructiveHint=False,
        idempotentHint=True,
...
@require_google_service("gmail", "gmail_read")
async def list_gmail_send_as(service, user_google_email: str) -> Dict[str, Any]:
    """List configured Gmail send-as identities."""
    response = await asyncio.to_thread(
        service.users().settings().sendAs().list(userId="me").execute
    )
    identities = response.get("sendAs", []) if response else []
    return {"count": len(identities), "sendAs": identities}


def _require_send_as_confirmation(
    action: str,
    send_as_email: str,
    dry_run: bool,
    confirmation: Optional[str],
) -> str:
    expected = f"{action}:{send_as_email}"
    if not dry_run and confirmation != expected:
        raise UserInputError(
            f"Refusing send-as mutation without exact confirmation '{expected}'"
        )
    return expected


@server.tool(
    title="Update Gmail Send-As Identity",
    annotations=ToolAnnotations(
        readOnlyHint=False,
        destructiveHint=False,
        idempotentHint=True,
...
@handle_http_errors("update_gmail_send_as", is_read_only=False, service_type="gmail")
...
async def update_gmail_send_as(
    ...
    """Preview or update the writable fields of one send-as identity."""
    expected = _require_send_as_confirmation(
        "UPDATE", send_as_email, dry_run, confirmation
    )
    body: Dict[str, Any] = {}
    ...
    if make_default:
        body["isDefault"] = True
    if not body:
        raise UserInputError("At least one writable send-as field is required")
    if dry_run:
        return {
            "dryRun": True,
    ...
    result = await asyncio.to_thread(
        ...
        .settings()
        .sendAs()
        .update(userId="me", sendAsEmail=send_as_email, body=body)
        .execute
    )
    return {"dryRun": False, "sendAs": result}


@server.tool(
    title="Delete Gmail Send-As Identity",
    annotations=ToolAnnotations(
        readOnlyHint=False,
...
@handle_http_errors("delete_gmail_send_as", is_read_only=False, service_type="gmail")
@require_google_service("gmail", "gmail_settings_sharing")
async def delete_gmail_send_as(
    ...
    """Preview or delete a non-primary send-as identity."""
    if send_as_email.casefold() == user_google_email.casefold():
        raise UserInputError(
            "The authenticated account's primary identity cannot be deleted"
        )
    expected = _require_send_as_confirmation(
        "DELETE", send_as_email, dry_run, confirmation
    )
    if dry_run:
        return {
            "dryRun": True,
    ...
    return {"dryRun": False, "deleted": send_as_email}
