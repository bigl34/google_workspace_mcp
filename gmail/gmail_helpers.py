...
def _normalize_email(address: str) -> str:
    ...
    e.g. 'Example User <user+foo@example.com>' normalizes to 'user@example.com'.

    This is the key primitive for 'is this message from the user?' checks -
    plus-addresses still belong to the user, not a third party.
    """
...
class _HTMLBlockTextExtractor(HTMLParser):
    ...
    with zero whitespace between them (e.g. "-Example" immediately followed by
    "Example User" with no break at all), which no amount of whitespace
