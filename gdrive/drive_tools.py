...
async def _download_file_bytes(
    ...
    _download_file_to_temp instead. `max_bytes` aborts mid-stream so a
    raw_base64 read cannot buffer more than the caller allowed.
    """
    fh = io.BytesIO()
    ...
    while not done:
        ...
        if max_bytes is not None and fh.getbuffer().nbytes > max_bytes:
...
async def get_drive_file_content(
    ...
    if raw_base64 and (max_bytes < 1 or max_bytes > 25 * 1024 * 1024):
        raise ValueError("max_bytes must be between 1 and 26214400")
    ...
    if export_format and mime_type.startswith("application/vnd.google-apps."):
        if export_format not in explicit_exports:
            raise ValueError("Unsupported export_format")
        export_mime_type = explicit_exports[export_format]
    else:
        export_mime_type = export_options.get(mime_type)

    file_content_bytes = await _download_file_bytes(
        service,
        file_id,
        export_mime_type,
        max_bytes=max_bytes if raw_base64 else None,
    )
    if raw_base64:
        if len(file_content_bytes) > max_bytes:
            raise ValueError(
                f"Downloaded file is {len(file_content_bytes)} bytes, exceeding max_bytes={max_bytes}"
            )
        return {
            "fileId": file_id,
            "fileName": file_name,
            ...
            "sizeBytes": len(file_content_bytes),
            "encoding": "base64",
