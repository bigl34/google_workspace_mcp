...
class TestNormalization:
    ...
    def test_plus_addressing_stripped(self):
        assert _normalize_email("user+foo@example.com") == "user@example.com"

    def test_display_name_stripped(self):
        assert (
            _normalize_email("Example User <user@example.com>")
            == "user@example.com"
        )
...
class TestBallInCourt:
    def test_vendor_replied_last_ball_on_user(self):
        thread = _thread(
            "t1",
            [
                _msg(
        ...
        result = _analyze_thread_ownership_impl(thread, "user@example.com")
        assert result["ball_in_court_of"] == "user"
        assert result["last_sender"] == "Vendor <vendor@example.com>"
        assert result["message_count"] == 2

    def test_user_replied_last_ball_on_them(self):
        thread = _thread(
            "t2",
            [
                _msg(
        ...
        assert result["last_sender"] == "Example User <user@example.com>"
    ...
    def test_plus_addressing_recognized_as_user(self):
        """user+foo@example.com is still Example User."""
        thread = _thread(
            "t3",
            [
                _msg(
                ...
                _msg(
                    "m2",
                    "Example User <user+newsletter@example.com>",
                    "Tue, 15 Apr 2026 10:00:00 -0400",
                ),
            ],
        )
...
class TestDrafts:
    def test_draft_excluded_from_last_determination(self):
        """A newer DRAFT from Example User must NOT flip ball-in-court; drafts
        haven't been sent."""
        thread = _thread(
            "t4",
            [
...
class TestParticipantsAndCounts:
    def test_three_party_thread(self):
        ...
        assert result["message_count_by_sender"]["user@example.com"] == 1
    ...
    def test_recipient_headers_are_case_insensitive_and_not_collapsed(self):
        ...
        result = _analyze_thread_ownership_impl(
            _thread("t-case", [message]), "user@example.com"
        )
...
class TestEmptyAndMalformed:
    def test_empty_thread(self):
        result = _analyze_thread_ownership_impl(
            {"id": "empty", "messages": []}, "user@example.com"
        )
