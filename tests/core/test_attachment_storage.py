...
class TestSaveAttachmentFromPath:
    ...
    def test_refreshes_destination_mtime_after_cross_filesystem_copy(
        self, storage, tmp_path, monkeypatch
    ):
        src = self._source(tmp_path)

        def long_cross_filesystem_move(source, destination):
            shutil.copy2(source, destination)
            past = time.time() - 7200
            os.utime(destination, (past, past))
            Path(source).unlink()
            return destination

        monkeypatch.setattr(
            attachment_storage.shutil, "move", long_cross_filesystem_move
        )
