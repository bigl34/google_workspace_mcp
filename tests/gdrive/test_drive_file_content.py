...
@pytest.mark.asyncio
async def test_download_file_bytes_enforces_optional_size_limit():
    mock_service = Mock()
    mock_service.files().get_media.return_value = "req"

    with _patch_downloader(b"too-large"):
        with pytest.raises(ValueError, match="max_bytes=4"):
            await _download_file_bytes(mock_service, "file123", max_bytes=4)
...
@pytest.mark.asyncio
async def test_get_drive_file_content_raw_is_explicit_bounded_base64(
    mock_resolve_image,
):
    image_bytes = b"\x00\x01binary"
    mock_service = Mock()
    mock_service.files().get_media.return_value = "req"
    ...
    assert result["sizeBytes"] == len(image_bytes)
