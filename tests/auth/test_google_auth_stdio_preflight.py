...
@pytest.mark.asyncio
async def test_noninteractive_missing_credentials_returns_auth_required_without_oauth(
    monkeypatch,
):
    ...
    def fail_if_called(*args, **kwargs):  # noqa: ARG001
        raise AssertionError("noninteractive reads must not start OAuth")

    monkeypatch.setenv("WORKSPACE_MCP_NONINTERACTIVE", "1")
    ...
    monkeypatch.setattr("auth.google_auth.get_oauth_redirect_uri", fail_if_called)
    monkeypatch.setattr("auth.google_auth.start_auth_flow", fail_if_called)
    monkeypatch.setattr(
    ...
    with pytest.raises(
        ...
        match=r"AUTH_REQUIRED account=user@gmail\.com service=gmail tool=test_tool",
    ):
